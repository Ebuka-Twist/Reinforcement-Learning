...
for episode in range(num_episodes):
    ...
    while not done:
        ...
        if len(replay_buffer) >= batch_size:
            ...
            q_values_next = target_network.predict(np.array(next_states))
            q_values_next_online = q_network.predict(np.array(next_states))
            target_q_values = rewards + discount_factor * (1 - np.array(dones)) * \
                              np.max(q_values_next_online, axis=1)
